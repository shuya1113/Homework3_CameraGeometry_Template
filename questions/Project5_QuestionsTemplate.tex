...
\begin{enumerate}[(a)]
\item Rewrite the equation $\boldsymbol{M}x \approx x'$ into a pair of linear equations in the form
\begin{align}
    \_\_m_{1,1} + \_\_m_{1,2} + \_\_m_{2,1} + \_\_m_{2,2} = \_\_
\end{align}
where the blanks are known coefficients or 0 (remember we know $x$, $y$, $x'$, and $y'$)
...
\end{align}
\emph{Hint:} you should have a pair of equations for each $x$-$x'$ correspondence, giving you $8$ rows in $\boldsymbol{Q}$ and $b$.
