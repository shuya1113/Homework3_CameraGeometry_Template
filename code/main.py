...
def main(args):
    ...
    if args.noise:
        Points_2D_pic_a = Points_2D_pic_a + 6 * np.random.rand(
            Points_2D_pic_a.shape[0], Points_2D_pic_a.shape[1]) - 0.5
        Points_2D_pic_b = Points_2D_pic_b + 6 * np.random.rand(
            Points_2D_pic_b.shape[0], Points_2D_pic_b.shape[1]) - 0.5
    ...
    if args.image == "rushmore":
        pic_a = io.imread(data_dir +
                          'Mount Rushmore/9193029855_2c85a50e91_o.jpg')
        pic_b = io.imread(data_dir +
                          'Mount Rushmore/7433804322_06c5620f13_o.jpg')
        pic_a = transform.rescale(pic_a, 0.25, order=1)
        pic_b = transform.rescale(pic_b, 0.37, order=1)
    ...
    # good job of filtering spurious matches.
        ...
        pic_a = transform.rescale(pic_a, 0.5, order=1)
        pic_b = transform.rescale(pic_b, 0.5, order=1)
    ...
    # seems to make this pair work much better.
        ...
        pic_a = io.imread(data_dir +
                          'Episcopal Gaudi/3743214471_1b5bbfda98_o.jpg')
        pic_b = io.imread(data_dir +
                          'Episcopal Gaudi/4386465943_8cf9776378_o.jpg')
        pic_a = transform.rescale(pic_a, 0.8, order=1)
        pic_b = transform.rescale(pic_b, 1.0, order=1)
    ...
    elif args.image == "woodruff_dorm":
        pic_a = io.imread(data_dir + 'Woodruff Dorm/wood1.jpg')
        pic_b = io.imread(data_dir + 'Woodruff Dorm/wood2.jpg')
        pic_a = transform.rescale(pic_a, 0.65, order=1)
        pic_b = transform.rescale(pic_b, 0.65, order=1)
    ...
    draw_epipolar_lines(F_matrix, pic_a, pic_b, matched_points_a,
                        matched_points_b)

    # optional - re estimate the fundamental matrix using ALL the inliers.
    if args.use_all:
        F_matrix = estimate_fundamental_matrix(matched_points_a,
                                               matched_points_b)
        draw_epipolar_lines(F_matrix, pic_a, pic_b, matched_points_a,
                            matched_points_b)
